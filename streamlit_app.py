...
with tab_policy:
    st.subheader("Ask a banking policy question")
    st.caption(
        "Retrieval-augmented answers grounded in Basel and FATF source documents. "
        "Answers cite passages inline as [1], [2]; out-of-scope questions are refused "
        "rather than answered from the model's own knowledge."
    )

    EXAMPLES = [
        "What does Basel require for operational risk capital?",
        "What are the FATF recommendations on customer due diligence?",
        "How should banks handle politically exposed persons?",
        "What is the best pizza topping?",  # deliberately out of scope
    ]
    example = st.selectbox(
        "Try an example (or write your own below)", [""] + EXAMPLES, key="pol_ex"
    )
    ...
    k = st.slider("Passages to retrieve (k)", 1, 10, 4, key="pol_k")

    if st.button("Ask", key="pol_btn", type="primary"):
        if len(question.strip()) < 3:
            ...
            try:
                ...
                if r.status_code == 503:
                    st.error("RAG index not found on the server. Build the index first.")
                else:
                    st.error(f"Request failed: {e}")
            except Exception as e:
                st.error(f"Request failed: {e}")
